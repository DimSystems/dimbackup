...
import { loadCategory, loadChannel } from "../utils.js";
...
/* restore the guild channels */
export async function loadChannelsSpace(guild, backup, options, limiter) {
    for (let category of backup.channels.categories) {
        const createdCategory = await loadCategory(category, guild, limiter);
        for (let channel of category.children) {
            await loadChannel(channel, guild, createdCategory, options, limiter).then(() => {
                return;
            });
        }
    }
