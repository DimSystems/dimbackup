...
import { fetchChannelPermissions, fetchTextChannelData, fetchVoiceChannelData } from "../utils.js";
...
/* returns an array with the channels of the guild */
export async function getChannelsSpace(guild, options, limiter, spaceId) {
    const channel = await limiter.schedule({ id: "getChannels::guild.channels.fetch" }, () => guild.channels.cache.get(spaceId));
    const collectedChannels = { categories: [], others: [] };
    
    const categoryData = { name: channel.name, permissions: fetchChannelPermissions(channel), children: [] };

    const children = channel.children.cache.sort((a, b) => a.position - b.position).toJSON();
